...
#
#SELECT DISTINCT language FROM city,country cc,country rc,countrylanguage cl WHERE city.name="riga" AND 
#city.countrycode=cc.code AND cc.region=rc.region AND rc.code=cl.countrycode;
# 20: Get the name of the most populous city
#SELECT name FROM city tc WHERE tc.population=(SELECT max(population) FROM city);
#SELECT cc.name,countrylanguagecountrylanguagecc.population,max(mc.population) mp FROM city cc,city mc GROUP BY cc.name HAVING cc.population=mp;
#
